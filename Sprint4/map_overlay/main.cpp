...
int main(int argc, char** argv )
{
    ...
    cv::resize(world_Map, world_Map, cv::Size(default_y, default_x), cv::INTER_AREA);
    ...
    cv::resize(world_gray, world_gray, cv::Size(default_y, default_x), cv::INTER_AREA);
    ...
    cv::Mat scaled_slam;
    cv::resize(slam_Map, scaled_slam, cv::Size(new_y, new_x), cv::INTER_AREA);

    cv::resize(scaled_slam, scaled_slam, cv::Size(default_y-100, default_x-100), cv::INTER_AREA);

    //cv::resize(new_slam, scaled_slam, cv::Size(), cv::INTER_AREA);
